...
class MailNewsParser(BaseParser):
    """Parser for Mail.ru News"""

    def __init__(self):
        super().__init__(
            source_name="Mail.ru News",
            source_url="https://news.mail.ru"
        )

    def _get_url(self, param_dict: dict) -> str:
        """Build URL for Mail.ru News category"""
    ...
    def _is_relevant_url(self, url: str) -> bool:
        ...
        irrelevant_patterns = ['/photo/', '/video/', '/live/']
    ...
    def _get_article_data(self, url: str) -> tuple:
        ...
        try:
            ...
            # Mail.ru specific selectors
            title_elem = soup.find('h1', {'class': 'hdr__text'})
            title = title_elem.text.strip() if title_elem else "No title"

            content_elem = soup.find('div', {'class': 'article__text'})
            if content_elem:
    ...
    def _get_category_articles(self, category: str = 'politics') -> pd.DataFrame:
        """Fetch articles from Mail.ru News category page"""
        try:
            ...
            # Mail.ru specific article selectors
            news_items = soup.find_all('div', {'class': 'newsitem'})

            for item in news_items[:20]:  # Limit to 20 articles
                link_elem = item.find('a', {'class': 'newsitem__title'})
                if link_elem:
                    article_url = link_elem['href']
                    if not article_url.startswith('http'):
                        article_url = f"https://news.mail.ru{article_url}"

                    title = link_elem.text.strip()

                    time_elem = item.find('span', {'class': 'newsitem__param'})
    ...
    async def fetch_news(
        ...
        """
        Fetch recent news from Mail.ru News
        ...
        if hours_back == 0:
            ...
            log.info(f"Fetching Mail.ru News for today: {date_from.strftime('%d.%m.%Y')}")
        else:
            date_from = now - timedelta(hours=hours_back)
            log.info(f"Fetching Mail.ru News (hours_back={hours_back})")
        ...
        log.info(f"Total fetched: {len(all_articles)} articles from Mail.ru News")
